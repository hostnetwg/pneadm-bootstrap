...
- `pnedu` `ProductCheckoutTest`: GET `/kursy/{slug}/zamowienie?price=` nie może dostać 500 po odfiltrowaniu wariantów bezpłatnych; checkout ma 4 kroki (Profil / Kontakt / Faktura / Płatność) jak formularz szkolenia V2; nowy zakup startuje od profilu „Osoba prywatna”; przy osobie prywatnej jest przełącznik kopiowania danych zamawiającego do uczestnika i faktury.
