...
| Belka zasobów na `/transmisja` (panel live ADM) | `--filter=CourseLivePanelTest` (przełączniki; zaświadczenie; lista **teraz na live**; rejestracja zaparkowana); **pnedu:** `--filter=LiveTransmissionResourceBar`, `--filter=LiveEmbedPresence`, `--filter=LiveTransmissionMeetingStatusResourceBar`; kanon: [LIVE_EMBED_RESOURCE_BAR.md](./LIVE_EMBED_RESOURCE_BAR.md) |
