...
| Belka zasobów na `/transmisja` (panel live ADM) | `--filter=CourseLivePanelTest` (przełączniki nieaktywne bez zasobu; czwarty: Pobierz zaświadczenie; rejestracja listy obecności zaparkowana na osadzonym live); **pnedu:** `--filter=LiveTransmissionResourceBar`, `--filter=LiveTransmissionMeetingStatusResourceBar`; kanon: [LIVE_EMBED_RESOURCE_BAR.md](./LIVE_EMBED_RESOURCE_BAR.md) |
