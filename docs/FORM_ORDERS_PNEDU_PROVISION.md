...
Dla nowego konta (uczestnik nie miał jeszcze hasła na pnedu.pl) przycisk **Ustaw hasło na pnedu.pl** jest pokazany przed sekcją spotkania. Link ustawienia hasła dostaje `redirect=/dashboard/szkolenia`; po poprawnym ustawieniu hasła pnedu automatycznie loguje użytkownika i otwiera listę szkoleń (przycisk „Dołącz do spotkania na żywo” aktywuje się 2 h przed startem — nie otwieramy pokoju od razu).
...
- **Embed w mailu włączony:** dla istniejącego konta główny CTA = `PNEDU_FRONTEND_URL/dashboard/szkolenia/{participant}/transmisja?fullscreen=1`; dla nowego konta CTA = `/ustaw-haslo/{token}?email=…&redirect=/dashboard/szkolenia` (lista szkoleń, nie pokój). Bezpośredni link CM zostaje jako alternatywa w treści.
...
- **Wyślij link do live** — e-mail systemowy (`ParticipantLiveMeetingLinkNotification`, log `certificate_email_logs.type = live_meeting_link`). Temat: `Dostęp do szkolenia: {tytuł} - spotkanie na żywo.` Przy embed (checkbox linku w mailu): notka → **Zaloguj się** (`/login?email=…`) → linki embed + fallback CM; bez embed: sekcja live + przycisk **Dołącz do spotkania na żywo**. Przed wysyłką odświeża `room_url` z API ClickMeeting (tak samo podgląd / ponowna wysyłka maila provisionu). Szczegóły: `pnedu/docs/DASHBOARD_LIVE_EMBED.md`.
- **CM pnedu (lista uczestników)** — gdy kurs ma tryb osadzonego pokoju, uczestnik z wejściem przez `/transmisja` dostaje badge **CM pnedu** w kolumnie **Data wygaśnięcia dostępu**. Zapis: `participant_live_access.embed_first_entered_at` / `embed_last_entered_at` (pnedu przy GET `/transmisja`).
