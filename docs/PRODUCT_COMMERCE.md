...
`Pokazuj w katalogu` publikuje kartę na `/kursy`. Żeby dało się kupić, potrzebne są jeszcze `Można kupić`, co najmniej jeden sposób płatności i aktywny wariant ceny. Kurs tylko w katalogu (sprzedaż zamknięta) ma na pnedu.pl wyszarzony, nieaktywny przycisk „Sprzedaż zamknięta”, na stronie oferty komunikat „Sprzedaż wyłączona”, `noindex` i nie trafia do sitemapy. Kolejność kart ustawiasz na `/online-courses` (przeciągnij wiersz albo strzałki). Publiczny katalog najpierw pokazuje sprzedaż otwartą, potem wyłączoną, w tej kolejności.
...
- nieopłacone zamówienie produktowe przez bramkę ma w ADM przycisk **Wyślij mail recovery płatności** (tylko „Zapłać ponownie”, bez FV odroczonej i bez crona — cron zostaje przy szkoleniach live); brak próby w `online_payment_orders` nie blokuje wysyłki,
