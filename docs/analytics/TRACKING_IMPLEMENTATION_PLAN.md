...
**Etap B (JS tracking):** wdrożono produkcyjnie **B1** + **B1a** (`pnedu` `6b32a4d`), **B2** (`pnedu` `bdc74ca`), **B3** (`pneadm` `b0b4535`, agregacja porzuceń, 2026-06-25) oraz **linki w sales-funnel** (`pneadm` `60acc21`). **B4** (dashboard porzuceń, read-only, agregaty B3) — wypchnięte (`pneadm` `a6ee852`). **B5** (CSV AI-safe export z dashboardu porzuceń, agregaty B3/B4, bez raw eventów/sesji/PII) — zaimplementowane w `pneadm`, czeka na deploy. Pełny opis: `docs/analytics/STAGE_B_CLIENT_TRACKING.md`.
