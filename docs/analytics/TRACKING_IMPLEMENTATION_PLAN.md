...
**Etap B (JS tracking) — start:** wdrożono lokalnie **B1** (endpoint `POST /analytics/client-events` w `pnedu`: batch ≤20, payload ≤10 KB, rate limit 60/min/IP, fail-silent `204`, 4 eventy MVP, whitelisty wartości, tryby), **B1a hardening** (same-origin guard po HOŚCIE z `Origin`/`Referer`; klientowski `event_uuid` jako wyłącznie seed deduplikacji, finalny `event_uuid` = deterministyczny UUIDv5 namespacowany sesją formularza i nazwą eventu; CSRF-exempt zostaje dla `sendBeacon`; batch > limitu ucinany do limitu) oraz **B2 — JS collector** (inline, fail-silent, ładowany tylko na stronie formularza zamówienia; `data-analytics-section`/`data-analytics-cta`; batch ≤20, debounce ~3 s, flush `submit`/`visibilitychange`/`pagehide` przez `sendBeacon`+`fetch keepalive`; zero wartości pól; nie blokuje submitu; tryby egzekwuje backend; **czeka na zgodę na commit**). Porzucenia nadal poza zakresem B1/B1a/B2. Pełny opis: `docs/analytics/STAGE_B_CLIENT_TRACKING.md`. Do zrobienia: **B3** (agregacja porzuceń po 24 h), **B4** (dashboard porzuceń).
