...
| `order_form_viewed` | wejście w formularz | `course_id`, `campaign_code`, `landing_target`, `price_variant_id`, `metadata.form_variant` (`legacy`\|`v2`) | `full`, `standard`, `light`, `aggregate_only` |
| `order_form_submit_attempted` | submit formularza przed zapisem | `course_id`, `campaign_code`, `payment_type`, `buyer_type`, `participant_count`, `metadata.form_variant` | `full`, `standard`, `light` |
| `order_form_validation_failed` | nieudana walidacja backendowa | `course_id`, `field_keys`, `error_rules`, `buyer_type`, `payment_type`, `metadata.form_variant` | `full`, `standard` |
...
| `online_payment_selected` | wybór płatności online po udanej walidacji (wdrożone w Etapie 2A-1) | `course_id`, `payment_type=online`, `payment_gateway` (`payu`/`paynow`/`unknown`), `buyer_type`, `has_price_variant`, `order_flow=online`, `metadata.form_variant` | `full`, `standard`, `light` |
| `deferred_invoice_selected` | wybór faktury / płatności odroczonej po udanej walidacji (wdrożone w Etapie 2A-1) | `course_id`, `payment_type=deferred_invoice`, `buyer_type`, `has_price_variant`, `order_flow=deferred`, `metadata.form_variant` | `full`, `standard`, `light` |
| `payment_order_created` | utworzenie `OnlinePaymentOrder` (wdrożone w Etapie 2A-2) | `payment_order_id`, `form_order_id`, `payment_gateway`, `amount_snapshot`, `buyer_type`, `has_price_variant`, `order_flow=online`, `metadata.form_variant` | `full`, `standard`, `light` |
...
>
> **Formularz zamówienia UI V2 + brama URL (2026-07-10):** równoległy kreator (`legacy` vs `v2`) z bramą `GET /courses/{id}/order-form`. Backend zapisuje `metadata.form_variant` przy `order_form_viewed`, submit, walidacji, `form_order_created` oraz eventach wyboru płatności (`online_payment_selected`, `deferred_invoice_selected`, `payment_order_created`). JS collector B2 **nie** wysyła `form_variant` w batchu — wariant wynika z backendu (brama, trasa POST, hidden field). Dokumentacja: `pnedu/docs/ORDER_FORM_V2.md`.
