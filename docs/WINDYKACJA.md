...
- **Wiele przelewów → jedna FV/sprawa (dopłaty / składki):** saldo sprawy = `amount_gross − suma(allocated accepted)`. Alokacja = `min(wolne na przelewie, brakujące na FV)`. Nadpłata (FV już w pełni pokryta) — **zablokowana** jako zwykła akceptacja. **Podwójna wpłata / zwrot (wariant B):** gdy sprawa jest już pokryta, drugi przelew można oznaczyć **„Oznacz jako zwrot”** (`status=refunded`, `allocated_amount=0`, powód `duplicate_overpayment`) — widoczny przy sprawie (badge Zwroty), **nie** liczy się do pokrycia, **bez** iFirma, znika z kolejki przeglądu; zakładka importu **Zwroty**. Cofnięcie: „Cofnij zwrot”. Historia: `DebtCaseAction::TYPE_BANK_REFUND`. Route: `POST accounting.collections.bank-transactions.mark-refunded`.
