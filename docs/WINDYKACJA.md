...
- Ręczne powiązanie od strony przelewu: w modalu podglądu importu panel „Powiąż ręcznie” (`GET accounting.bank-imports.lookup-cases` — niezamknięte sprawy + zamówienia bez aktywnej sprawy; FV/KSeF/NIP/nazwa/e-mail/ID) + `POST .../transactions/{tx}/link-case` (`debt_case_id` albo `form_order_id`; dla zamówienia tworzy sprawę). Ikona oka przy wyniku (zamówienie lub sprawa z `form_order_id`) ładuje podgląd do prawej kolumny (`GET accounting.bank-imports.lookup-order-preview`); aktywne oko wyróżnione; **Wyczyść** przywraca oryginalną sugestię przelewu. Dla zaakceptowanego lokalnie dopasowania można ponowić samą rejestrację wpłaty przez `POST .../matches/{match}/register-ifirma-payment`.
