...
- Ręczne powiązanie od strony sprawy: `GET accounting.collections.show` przyjmuje `bank_search` / `bank_amount` / `bank_after_order` i pokazuje wpływy bez zaakceptowanego/ignorowanego matcha; domyślnie tylko przelewy z `operation_date >= order_date`; `POST accounting.collections.bank-transactions.link` tworzy ręczny match (`manual_case_link`) i akceptuje go dla bieżącej sprawy.
- Ręczne powiązanie od strony przelewu: w modalu podglądu importu panel „Powiąż ręcznie” (`GET accounting.bank-imports.lookup-cases` — niezamknięte sprawy + zamówienia bez aktywnej sprawy; FV/KSeF/NIP/nazwa/adres/miasto/e-mail/ID, bez rozróżniania wielkości liter) + `POST .../transactions/{tx}/link-case` (`debt_case_id` albo `form_order_id`; dla zamówienia tworzy sprawę). Ikona oka przy wyniku (zamówienie lub sprawa z `form_order_id`) ładuje podgląd do prawej kolumny (`GET accounting.bank-imports.lookup-order-preview`); aktywne oko wyróżnione; **Wyczyść** przywraca oryginalną sugestię przelewu. Dla zaakceptowanego lokalnie dopasowania można ponowić samą rejestrację wpłaty przez `POST .../matches/{match}/register-ifirma-payment`.
