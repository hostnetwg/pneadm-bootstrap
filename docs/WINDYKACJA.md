...
- **Wiele przelewów → jedna FV/sprawa (dopłaty / składki):** saldo sprawy = `amount_gross − suma(allocated accepted)`. Alokacja = `min(wolne na przelewie, brakujące na FV)`. Nadpłata (FV już w pełni pokryta) — **zablokowana** jako zwykła akceptacja. **Podwójna wpłata / zwrot (wariant B):** gdy sprawa jest już pokryta (także **zamknięta**), drugi przelew można oznaczyć **„Oznacz jako zwrot”**: na karcie sprawy (jak dotychczas) oraz w **stopce modala podglądu importu** obok Odrzuć/Ignoruj (Akceptuj jest wtedy ukryty, bo i tak padnie). Dodatkowo przycisk w prawej kolumnie / po ręcznym wyszukaniu (oko). Status `refunded`, `allocated_amount=0`, powód `duplicate_overpayment` — widoczny przy sprawie (badge Zwroty), **nie** liczy się do pokrycia, **bez** iFirma, znika z kolejki; zakładka importu **Zwroty**. Cofnięcie: „Cofnij zwrot”. Historia: `DebtCaseAction::TYPE_BANK_REFUND`. Route’y: `POST accounting.collections.bank-transactions.mark-refunded`, `POST accounting.bank-imports.transactions.mark-refunded`.
