...
- Ręczne powiązanie od strony sprawy: `GET accounting.collections.show` przyjmuje `bank_search` / `bank_amount` i pokazuje wpływy bez zaakceptowanego/ignorowanego matcha; `POST accounting.collections.bank-transactions.link` tworzy ręczny match (`manual_case_link`) i akceptuje go dla bieżącej sprawy.
