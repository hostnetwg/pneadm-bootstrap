...
- Filtry przeglądu: `Do przeglądu`, `Bez powiązania`, `High`, `Medium`, `Low`, `Zaakceptowane`, **`Zwroty`**, **`Na potem`** (czerwona zakładka gdy > 0), `PayNow`, `Ignorowane`, `Wszystkie wpływy`.
- **Wyszukiwarka przelewów** na stronie importu (`?q=`): działa w bieżącej zakładce (najużyteczniejsza przy `filter=all`); szuka w opisie/nadawcy/rachunku, kwocie (`365` / `365,00`), dacie (`2026-08-25` / `25.08.2026`), ID przelewu (`#5393`) oraz powiązanej FV/KSeF/nazwie nabywcy/odbiorcy. Liczniki zakładek zostają bez zmian. Fraza zachowywana przy zmianie zakładki, paginacji i po akceptacji/powiązaniu.
- **Na potem:** przycisk obok „Ignoruj” odkłada przelew poza aktywną kolejkę; nadal liczy się do „Do przeglądu” na liście importów. Z zakładki „Na potem” można od razu **powiązać / akceptować** jak w kolejce (bez „Przywróć do przeglądu”); status match: `deferred`.
- **Komunikaty w modalu podglądu:** błędy powiązania/akceptacji (np. FV już w pełni pokryta) wracają z `?preview=` i pokazywane są wewnątrz okna podglądu — bez zamykania okna i bez ukrytego flasha na stronie w tle.
...
- **Podział jednego przelewu na kilka FV:** kolumna `bank_transaction_matches.allocated_amount`; wiele `accepted` na jednym wpływie, o ile suma alokacji ≤ kwota przelewu (±0,01). Alokacja = min(kwota FV / brakujące na FV, wolne na przelewie). Częściowe pokrycie dozwolone (reszta może zostać otwarta). Matcher: gdy w tytule ≥2 numery FV i suma kwot zamówień ≈ przelew → High + `multi_invoice_sum_match` + `amount_match`. UI: lista FV pakietu, **Akceptuj pakiet**, **Dodaj do podziału**, iFirma per alokacja. Route: `POST accounting.collections.bank-matches.register-ifirma`.
- **Wiele przelewów → jedna FV/sprawa (dopłaty / składki):** saldo sprawy = `amount_gross − suma(allocated accepted)`. Alokacja = `min(wolne na przelewie, brakujące na FV)`. Nadpłata na **tę samą** FV (już w pełni pokryta) — zwykła akceptacja sugestii **padnie**. **Podwójna wpłata / zwrot (wariant B):** gdy sprawa jest już pokryta (także **zamknięta**), drugi przelew można oznaczyć **„Oznacz jako zwrot”** (karta sprawy + stopka i prawa kolumna modala importu). **Akceptuj zostaje** — tytuł przelewu bywa skopiowany ze starej FV przy płatności za inne zamówienie; wtedy wyszukaj właściwe zamówienie (oko) i **Akceptuj / Powiąż** wiąże z kandydatem, nie ze starą FV. Status `refunded`, `allocated_amount=0`, powód `duplicate_overpayment` — widoczny przy sprawie (badge Zwroty), **nie** liczy się do pokrycia, **bez** iFirma, znika z kolejki; zakładka importu **Zwroty**. Cofnięcie: „Cofnij zwrot”. Historia: `DebtCaseAction::TYPE_BANK_REFUND`. Route’y: `POST accounting.collections.bank-transactions.mark-refunded`, `POST accounting.bank-imports.transactions.mark-refunded`.
...
- Ikona oka przy wierszu → modal Bootstrap z porównaniem **przelew z wyciągu** ↔ **zamówienie/sugestia** (FV z opisu, KSeF z tytułu, nabywca, NIP, e-mail, podstawa dopasowania). W nagłówku podglądu: ile zostało **do akceptacji** w bieżącym filtrze (np. High) oraz łącznie **do przeglądu**; przy Poprzedni/Następny pozycja na stronie.
