...
- **Wiele przelewów → jedna FV/sprawa (dopłaty / składki):** saldo sprawy = `amount_gross − suma(allocated accepted)`. Alokacja = `min(wolne na przelewie, brakujące na FV)`. Nadpłata (FV już w pełni pokryta) — **zablokowana**. UI na karcie sprawy: badge pokrycia + domyślna kwota wyszukiwania = brakujące. iFirma: **każda** częściowa alokacja może być zarejestrowana (`allocated_amount`), o ile nie przekracza kwoty FV / pozostałego w iFirma.
