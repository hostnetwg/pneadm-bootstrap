...
class AnalyticsFormAbandonmentController extends Controller
{
    ...
    public function index(
        Request $request,
        AnalyticsFormAbandonmentDashboardService $dashboard,
        AnalyticsDateRangePresets $presets,
    ): View {
        $this->ensureEnabled();

        $data = $dashboard->build($request->only(self::FILTER_KEYS));
        // Presety celują w dane dojrzałe (kotwica = dziś − lag agregacji).
    ...
    /**
     * Ręczne przeliczenie agregatów porzuceń (B3) dla widocznego zakresu dat.
     * Idempotentne (serwis kasuje i liczy od zera per dzień), admin-only (middleware grupy).
     */
    public function recompute(
        Request $request,
        AnalyticsFormAbandonmentDashboardService $dashboard,
        AnalyticsAbandonmentAggregationService $aggregation,
    ): RedirectResponse {
        $this->ensureEnabled();

        $timezone = $aggregation->timezone();
        $lag = max(0, (int) config('analytics.abandonment.aggregation_lag_days', 2));
        ...
        $redirect = redirect()->route('analytics.form-abandonments.index', $this->preservedFilters($request));

        $maxDays = max(1, (int) config('analytics.form_abandonment_dashboard.recompute_max_days', self::DEFAULT_MAX_RECOMPUTE_DAYS));
        ...
        return $redirect->with('recompute_status', sprintf(
            'Przeliczono porzucenia: dni %d, wiersze kursów %d, wiersze kampanii %d (%s – %s).',
            count($result['dates']),
            $result['course_rows'],
    ...
    private function logRecomputeSafely(string $from, string $to, array $result): void
    {
        try {
            ActivityLog::logCustom(
                'analytics_abandonments_recomputed',
                'Ręczne przeliczenie agregatów porzuceń formularza z panelu.',
                [
                    'date_from' => $from,
                    'date_to' => $to,
