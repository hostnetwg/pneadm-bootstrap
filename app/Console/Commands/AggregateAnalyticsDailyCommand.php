...
class AggregateAnalyticsDailyCommand extends Command
{
    ...
    protected $description = 'Przelicza dzienne agregaty analityczne z analytics_events do analytics_daily_*_stats.';

    public function handle(AnalyticsDailyAggregationService $aggregationService): int
    {
        ...
        try {
            if ($this->option('date')) {
                ...
                $result = $aggregationService->aggregateForDate($statDate);
                $this->line(sprintf(
                    'Brak opcji daty — użyto domyślnie dnia poprzedniego (%s, %s).',
        ...
        $this->info(sprintf(
            'Agregacja zakończona. Dni: %d, wiersze kursów: %d, wiersze kampanii: %d.',
            count($result['dates']),
            $result['course_rows'],
