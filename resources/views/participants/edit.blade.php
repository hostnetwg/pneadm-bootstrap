    ...
    <x-slot name="header">
        <div>
            <h2 class="fw-semibold fs-4 text-dark mb-1">
                <i class="fas fa-user-edit me-2"></i>{{ __('Edycja danych uczestnika') }}
            </h2>
            ...
            <form action="{{ route('participants.update', ['course' => $participant->course_id, 'participant' => $participant->id]) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $participant->email) }}">
                        @error('email')
