...
class PneduFrontendCacheInvalidationServiceTest extends TestCase
{
    ...
    public function test_posts_survey_settings_invalidation(): void
    {
        config([
            'services.pnedu.internal_url' => 'http://pnedu-app',
            'services.pnedu.internal_api_token' => 'test-internal-token',
        ...
        Http::fake([
            'http://pnedu-app/api/internal/cache/survey-settings' => Http::response(['ok' => true], 200),
        ]);

        app(PneduFrontendCacheInvalidationService::class)->invalidateSurveySettings();

        Http::assertSent(function ($request) {
            return $request->url() === 'http://pnedu-app/api/internal/cache/survey-settings'
                && $request->hasHeader('Authorization', 'Bearer test-internal-token');
        });
    }
